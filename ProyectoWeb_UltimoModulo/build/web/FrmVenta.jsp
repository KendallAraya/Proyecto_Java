...
<html>
    ...
    <body>
        ...
        <div class="container bg-custom">
            <div class="row">
                <div class="col-10"><h1 class="text-center">Facturacion de Ventas</h1></div>
            </div>
            <br>
            <% 
                Venta venta = new Venta();
            %>
            <form action="FacturarVenta" method="post" id="form_AgregarVenta">
                <div id="FacturaVenta">
                    <div class="form-group float-right">
                        <div class="input-group">
                            <label for="txtNumFactura">Num Factura</label>
                            <input type="text" id="txtNumFactura" name="txtNumFactura" value="<%= venta.getIdVenta()%>" class="form-control" readonly>
                        </div>


                        <div class="input-group">
                            <label for="txtFechaFactura">Fecha Factura</label>
                            <input type="text" id="txtFechaFactura" name="txtFechaFactura" value="<%= venta.getFecha()%>" required class="datepicker form-control">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="input-group">
                            <input type="hidden" id="txtIdFuncionario" name="txtIdFuncionario" value="<%= venta.getIdFuncionario()%>" class="form-control" placeholder="ID. Funcionario" readonly>
                    ...
                    <div class="form-group">
                        <div class="input-group">
                            <input type="hidden" id="txtIdCliente" name="txtIdCliente" value="<%= venta.getIdCliente()%>" class="form-control" placeholder="ID. Cliente" readonly>
                            <input type="text" id="txtNombreCliente" name="txtNombreCliente" value="" class="form-control" placeholder="Seleccione un Cliente" readonly>
                            <a id="btnBuscarCliente" class="btn btn-success btnBuscar" data-toggle="modal" data-target="#buscarClientes">
                                <li class="fas fa-search"></li>
                            </a>
                        </div>
                    </div>
                    <div>
                        ...
                        <select id="selMetodoPago" name="selMetodoPago" class="form-control">
                            <option value="Efectivo" <%= venta.getMetodoPago().equals("Efectivo") ? "selected" : "" %>>Efectivo</option>
                            <option value="Tarjeta" <%= venta.getMetodoPago().equals("Tarjeta") ? "selected" : "" %>>Tarjeta</option>
                            <option value="Transaccion" <%= venta.getMetodoPago().equals("Transaccion") ? "selected" : "" %>>Transaccion</option>
                            <option value="SINPE" <%= venta.getMetodoPago().equals("SINPE") ? "selected" : "" %>>SINPE</option>
                        </select>
                    </div>
                </div>
                <hr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#" data-dismiss="modal"
                                               onclick="SeleccionarFuncionario('<%= codigoF %>', '<%= nombreF %> ')">Seleccionar</a>
                                        </td>
                                    </tr>
                                <% } %>
                            </tbody>
        ...
        <div class="modal" id="buscarClientes" tabindex="1" role="dialog" aria-labelledby="tituloVentana">
            <div class="modal-dialog" role="document" >
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <table id="tablaCliente">
                            ...
                            <tbody>
                                <% 
                                    ...
                                    List<Cliente> datoC;
                                    datoC=logicaC.ListarClientes_List("");
                                    for (Cliente registros : datoC) {

                                %>
                                    <tr>
                                        <% int codigoC = registros.getIdCliente();
                                           String nombreC = registros.getNombreCompleto();
                                        %>
                                        <!-- No termina con ; potque es una exprecion -->
                                        <td><%= codigoC %></td>
                                        <td><%= nombreC %></td>
                                        <td>
                                            <a href="#" data-dismiss="modal"
                                               onclick="SeleccionarCliente('<%= codigoC %>', '<%= nombreC %> ')">Seleccionar</a>
                                        </td>
                                    </tr>
                                <% } %>
                            </tbody>
        ...
        <script>        
            ...
            function SeleccionarFuncionario(idFuncionario, Nombre){
            ...
            function VerificarSeleccion() {
                ...
                if (nombreFuncionario !== "" && nombreCliente !== "") {
            ...
            $(document).ready(function() {
                ...
                $("#BtnAgregarProducto").click(function (e) {
                    ...
                    $.ajax({
                        ...
                        url: "FacturarVenta",  // URL del servlet
                        data: $("#form_AgregarVenta").serialize(),  // Datos del formulario serializados
                    ...
                    window.location.href = "frmDetalleVenta.jsp";
